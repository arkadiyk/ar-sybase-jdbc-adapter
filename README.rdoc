...
At the moment Sybase ASE version 15 or grater is required.
...
If <tt>.limit</tt> with no <tt>.offset</tt> or <tt>.count</tt> methods is used, the driver simply adds "TOP" keyword to SQL and send it to the Sybase server:
  User.limit(10)
produces:
  SELECT TOP 10 users.* FROM users

The driver has to rely on Java code to implement <tt>.offset</tt> or when <tt>.count</tt> is used together with <tt>.offset</tt> or <tt>.limit</tt>. In this case Rails generate SQL like it was MySQL query:
  User.limit(10).offset(20)
produces
  SELECT  users.* FROM users LIMIT 10 OFFSET 21
This may be confusing if you are looking at the log file.

Java takes this query, parses it and executes multistep scrollable cursor query:
