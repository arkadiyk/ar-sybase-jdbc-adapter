...
1) If the offset is a large number, the space taken 
   by the temptable can fill up the tempdb. 
...
1) Scrollable cursor works for Sybase ASE 
   starting from version 15. 
2) Cursors are not very efficient in Sybase ASE,
   and very inefficient in Sybase IQ.
